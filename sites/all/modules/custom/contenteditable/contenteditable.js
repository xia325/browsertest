(function ($) {
  var self = Drupal.behaviors.contenteditable = {
    attach: function (context) {
      ...
      $('div[contenteditable]', context).once('contenteditable').bind('focusin', self.focusin).bind('focusout', self.focusout);
    },
    v: function (obj) {
      var output = '';
    ...
    init: function () {
      ...
      var $controls = self.constructControls().add('<button>', {
        'id':
        'contenteditableCancel',
        'text': Drupal.t('Cancel'),
        click:
        self.hideControls,
        'data-tooltip': 'cancel edit'
      }).add('<button>', {
        'id': 'contenteditableSave',
        'text':
        Drupal.t('Save'),
        click: self.submitHandler,
        'data-tooltip':
        'save changes'
      });
    ...
    focusin: function (e) {
      //Move the controls to the right before the element we're editing,
      //but only when focusing in on a new field, otherwise do nothing.
    ...
    constructControls: function () {
      ...
      // Go over the settings object, construct the controls and return theme as 1 jQuery collection.
    ...
    submitHandler: function (e) {
      // If they manage to press the button before an element was set
      //as active,
      // or there are no controls, throw an exception, otherwise, post
      //the data.
      if (!self.active) {
      ...
      //disable contenteditable on active element until the ajax call
      //is complete
    ...
    successHandler: function (data, status, xhr) {
      ...
      $success.delay(2000).slideUp('slow', function () {
        $(this).remove();
      });
      //re-enable contenteditable on active element so it may be re-
      //edited
    ...
    hideControls: function () {
      ...
      self.controls.remove();
      self.controls = $clone;
      //unset self.currentField so that the same field may be re
      //-edited
      self.currentField = null;
    }
  };
